...
-- Helper to safely get player display name with fallback
...
-- Helper to safely get player username
...
-- Helper to safely load avatar thumbnail
...
function obj:Window(cfw)
	...
	-- FIXED: Use player's display name with fallback
	...
	-- FIXED: placeholder until async loads
	LogoPlayer.Image = ""
	...
	-- FIXED: Display name (nickname) as the main name
	...
	-- FIXED: Dynamically pull account age from Roblox
	local memberSince = "Unknown"
	task.spawn(function()
		local success, result = pcall(function()
			local userId = game.Players.LocalPlayer.UserId
			local info = game:GetService("Players"):GetPlayerInfoAsync(userId)
			return info
		end)
		if success and result and result.AccountAge then
			local days = result.AccountAge
			local createdTime = os.time() - (days * 86400)
			memberSince = os.date("%d %B %Y", createdTime)
		else
			-- fallback: just show account age in days
			local success2, age = pcall(function()
				return game.Players.LocalPlayer.AccountAge
			end)
			if success2 and age then
				memberSince = age .. " days ago"
			end
		end
		DiscordMemberSinceReal.Text = memberSince
	end)
	DiscordMemberSinceReal.Text = "Loading..."
	...
	-- FIXED: Load both avatars safely in background
	...
	DescriptionPlayer_3.Text = cfw.DiscordStatus
